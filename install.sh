...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/VLESS_tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/VLESS_h2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/dev/ssl_update.sh"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/whoisgit/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh)
        ;;
    4)
